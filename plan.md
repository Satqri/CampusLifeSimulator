# 校园生活模拟器 (Campus Life Simulator) — 开发计划 v2
...
│   ├── entity/
│   │   ├── Entity.h                # 实体抽象基类
│   │   ├── Character.h / .cpp      # 角色基类 (继承Entity)
│   │   ├── Player.h / Player.cpp   # 玩家 (继承Character)
│   │   └── Enemy.h / Enemy.cpp     # 敌人 (继承Character)
│   ├── map/
│   │   ├── Tile.h                  # 瓦片数据
...
### 继承树 1: 实体体系 (Entity → Character → Player/Enemy) 【3 层】

```
Entity                       (抽象基类)
  ├── # posX, posY           (位置，封装为 private)
  ├── # visible, active      (状态标志)
  ├── + update(dt) = 0       (纯虚函数 — 多态)
  ├── + render(window) = 0   (纯虚函数 — 多态)
  └── + getPos() const       (公共接口访问私有成员 — 封装)
        └── Character        (抽象基类)

# 校园生活模拟器 (Campus Life Simulator) — 开发计划 v4
...
│   ├── quest/                        # [NEW] 主线任务系统
│   │   ├── MainQuest.h / .cpp        # [DONE] 任务抽象基类
│   │   ├── SimpleQuest.h / .cpp      # [DONE] 简单任务（报到/选课/社团/毕业）
│   │   ├── ExamQuest.h / .cpp        # [DONE] 考试任务中间类
│   │   ├── MidtermExamQuest.h / .cpp # [DONE] 期中考试
│   │   ├── FinalExamQuest.h / .cpp   # [DONE] 期末考试
│   │   └── QuestManager.h / .cpp     # [DONE] 任务链管理器
│   ├── state/
│   │   ├── GameState.h             # [DONE] 状态抽象基类
│   │   ├── ExplorationState.h/.cpp # 探索状态
│   │   ├── EventDialogState.h/.cpp # 事件对话状态
│   │   ├── CombatState.h / .cpp    # 战斗状态
│   │   └── MainQuestState.h / .cpp  # [DONE] 主线任务状态
│   ├── fileio/
...
### 继承树 6: 主线任务体系 (MainQuest → ExamQuest → MidtermExamQuest/FinalExamQuest) 【3 层】 ✅ 已实现

```
MainQuest                     (抽象基类)
  ├── # questId, questName, description
  ├── # currentPhase: QuestPhase
  ├── # choiceTexts[], choiceOutcomes[]
  ├── # completionReward, questType
  ├── + execute(player) = 0   (纯虚 — 多态)
  ├── + render(window) = 0    (纯虚 — 多态)
  └── + handleInput(event, player, choiceMade) = 0
        ├── SimpleQuest        (具体类) — 配置驱动
        │     ├── 阶段: ANNOUNCEMENT → CHOICE → FINAL_RESULT
        │     ├── 用于: Orientation / CourseSelection / ClubActivity / Graduation
        │     └── execute() 应用选择 + 奖励属性变化
        └── ExamQuest          (抽象中间类)
              ├── # examSubject, subjectDC, totalRounds, currentRound
              ├── # score, reviewBonus, lastRoll: ExamRollResult
              ├── # performRoll(player)  (d20 检定)
              ├── # calculateResult() / applyResult(player)
              ├── 阶段: ANNOUNCEMENT → PREP_CHOICE → EXAM_ROUND ⇄ ROUND_RESULT → FINAL_RESULT
              │
              ├── MidtermExamQuest (具体类)  DC=14, 5轮, 需3次通过, 复习消耗20体力
              └── FinalExamQuest   (具体类)  DC=16, 7轮, 需4次通过, 复习消耗30体力
```

**新增类型:**
- `QuestPhase` 枚举: NOT_STARTED/ANNOUNCEMENT/CHOICE/PREPARATION/EXAM_ROUND/ROUND_RESULT/FINAL_RESULT/COMPLETED
- `MainQuestType` 枚举: ORIENTATION/COURSE_SELECTION/CLUB_ACTIVITY/MIDTERM_EXAM/FINAL_EXAM/GRADUATION
- `ExamRollResult` 结构体: { d20Roll, academicBonus, reviewBonus, total, dc, success }
- `StateType` 新增 `MAIN_QUEST` 状态
- 7 个主线任务配置位于 `assets/config/quests.json`

**QuestManager** — 独立工具类（组合于 Game）:
- 从 quests.json 加载任务链（事件计数阈值、任务参数、选项/奖励）
- 事件驱动制：完成 N 个随机事件后触发下一个主线任务
- 工厂方法 createNextQuest() 根据类型创建对应 MainQuest 子类
- getSemesterProgress() 返回 0.0~1.0 学期进度

**MainQuestState** — 新游戏状态（继承 GameState）:
- onEnter(): 创建当前任务，初始化 UI
- handleInput(): 委托给 currentQuest->handleInput()
- onExit(): currentQuest->execute(player) + QuestManager::onQuestCompleted()
- 支持多阶段渲染：公告/选择/复习/掷骰/轮次结果/最终结算

**OOP 体现:**
- **封装**: 所有任务数据为 private/protected，通过 public 接口访问
- **继承**: MainQuest → ExamQuest → MidtermExamQuest/FinalExamQuest，共 3 层
- **多态**: execute()/render()/handleInput() 为虚函数，SimpleQuest 和 ExamQuest 有不同实现

### 额外工具类 (满足 ≥5 类要求)

- `Dice` — 骰子掷骰工具类
- `LLMClient` — LLM API 客户端 (封装 HTTP 请求)
- `QuestManager` — 任务链管理器 (组合模式，持有 MainQuest 序列)
...
| `quests.json` | 读取 | 加载主线任务链配置（threshold/choices/reward） |
...
// 游戏状态
enum class StateType { EXPLORATION, EVENT_DIALOG, COMBAT, MAIN_QUEST, MENU, GAME_OVER };
```
...
```
        ...
        └──┬───────┬───┘
  踩中事件 │       │ SAN < 30 时概率触发 / 事件计数达阈值
           ▼       ▼
  ┌──────────┐  ┌──────────┐  ┌──────────┐
  │EVENT_DLG │  │ COMBAT   │  │MAIN_QUEST│
  │对话框+选项│  │回合制战斗│  │主线任务  │
  └────┬─────┘  └────┬─────┘  └────┬─────┘
       │              │              │ 任务完成
       ▼              ▼              ▼
...
### 阶段 3b: 主线任务系统 — 简单任务 (预估 2-3h) — ✅ 已完成

**产出文件:** `MainQuest.h/.cpp`, `SimpleQuest.h/.cpp`, `QuestManager.h/.cpp`, `MainQuestState.h/.cpp`, `GameState.h`, `quests.json`

- [x] `MainQuest` 抽象基类 + `SimpleQuest` 具体类 (2 层继承)
- [x] `QuestManager`: JSON 加载、事件计数、阈值检测、工厂方法
- [x] `MainQuestState`: 多阶段 UI (ANNOUNCEMENT → CHOICE → FINAL_RESULT)
- [x] `quests.json` 完整任务链配置（7 个任务）
- [x] `GameState` 抽象基类 stub
- [x] `StateType` 新增 MAIN_QUEST
- **验证:** 编译通过，所有 quest 文件无编译错误
...
### 阶段 6b: 主线任务系统 — 考试任务 (预估 2-3h) — ✅ 已完成

**产出文件:** `ExamQuest.h/.cpp`, `MidtermExamQuest.h/.cpp`, `FinalExamQuest.h/.cpp`

- [x] `ExamQuest` 考试中间类: ANNOUNCEMENT → PREPARATION → EXAM_ROUND ⇄ ROUND_RESULT → FINAL_RESULT
- [x] `MidtermExamQuest` / `FinalExamQuest` 具体配置 (3 层继承)
- [x] 复习机制: 消耗体力换取临时学业加值
- [x] 多轮 d20 检定: performRoll() + advanceRound() + calculateResult() + applyResult()
- [x] `ExamRollResult` 结构体 + `QuestPhase`/`MainQuestType` 枚举
- **验证:** 编译通过，考试系统完整代码无编译错误
...
| 5 | 存档 → 重启 → 读档 → 状态一致 |
| 6 | SAN低 → 战斗 → 骰子检定 → 结算 |
| 3b | 编译通过，所有 quest 文件无编译错误 |
| 6b | 编译通过，考试系统完整代码无编译错误 |

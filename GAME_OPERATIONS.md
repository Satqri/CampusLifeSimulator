...
- 清晨：`06:00-11:00`，使用标准外景亮度。
- 正午：`11:00-14:00`，外景会更亮。
- 下午：`14:00-17:30`，使用标准外景亮度。
- 傍晚：`17:30-18:30`，外景变暗并带傍晚色调，路灯开始发光。
- 夜晚：`18:30-06:00`，外景明显变黑，道路和建筑旁的路灯提供局部照明。
...
- 必须先在 `Get Food` 窗口选择并购买餐食，然后移动到 `Eat at Table` 座位上吃。
- 同一饭点只能打饭一次。午饭和晚饭分开计算。
- 玩家手上已经拿着餐盘时，不能再次打饭。
- 窗口打饭不消耗时间；坐在桌边吃饭固定消耗 `20min`。
- `Meal A`：价格 `Gold -8`，吃完 `SAN +3`、`Energy +12`、`Social +1`。
- `Meal B`：价格 `Gold -15`，吃完 `SAN +6`、`Energy +20`、`Social +2`。
- `Meal C`：价格 `Gold -28`，吃完 `SAN +10`、`Energy +30`、`Academic +2`、`Social +4`。
...
- `Play Games` 每次消耗 `60min`。每天前 `2` 次效果为 `SAN +12`、`Energy +8`；第 `3` 次及以后效果为 `SAN +4`、`Energy -12`、`Academic -2`。
- `Sit on Rug` 当前不消耗时间。

...
int Application::run() {
    ...
    TitleMenuScreen titleMenuScreen(titleScreen);
    SettingsMenuScreen settingsMenuScreen(settingsPanel);
    HelpMenuScreen helpMenuScreen(helpPanel);
    DifficultyMenuScreen difficultyMenuScreen(difficultyPanel);
    settingsPanel.setSettings(&gameSettings);
    settingsPanel.setOverlayMode(false);
    helpPanel.setOverlayMode(false);
    SceneBackground sceneBackground;
    SceneTransition sceneTransition;
    SceneTransitionScreen sceneTransitionScreen(font, sceneBackground, sceneTransition);
    ...
    GameState settingsReturnScreen = GameState::Title;
    ...
    auto openSettingsPanel = [&](GameState returnScreen) {
        settingsReturnScreen = returnScreen;
        settingsPanel.setEditing(false);
        settingsPanel.setOverlayMode(returnScreen == GameState::Game);
    ...
    auto closeSettingsPanel = [&]() {
        ...
        settingsReturnScreen = GameState::Title;
    };
    ...
    auto handleTitleAction = [&](TitleAction action) {
        switch (action) {
            ...
            case TitleAction::Settings:
                openSettingsPanel(GameState::Title);
                break;
            case TitleAction::Help:
                titleScreen.setVisible(false);
                screen = GameState::Help;
                break;
    ...
    // ── 主循环 ───────────────────────────────────────────────
    GamePlayScreen gamePlayScreen([&](sf::RenderWindow& targetWindow, TguiContext& targetTguiCtx) {
        targetWindow.clear(sf::Color(20, 20, 30));

        currentMap->render(targetWindow);
        player.render(targetWindow);

        if (fontOk) {
            renderStatsPanel(targetWindow, hud, player);
            timePanel.setTimeSystem(&timeSystem);
            timePanel.update(0.0f);
        }
        if (eventRunner.isActive()) {
            eventRunner.render(targetWindow, modalBox);
        }
        if (questActive) {
            MainQuest* quest = questManager.getCurrentQuest();
            if (quest && !quest->isCompleted()) {
                renderQuestContent(targetWindow, modalBox, quest);
            }
        }
        ...
        if (fontOk) {
            ...
            debugSandbox.render(targetWindow);
        }
    ...
    while (window.isOpen()) {
        ...
        sceneTransitionScreen.update(dt);
        ...
        while (const auto eventOpt = window.pollEvent()) {
            ...
            if (const auto* keyEv = event.getIf<sf::Event::KeyPressed>()) {
                if (screen == GameState::Game && isSettingsShortcut(*keyEv)) {
                    openSettingsPanel(GameState::Game);
                    continue;
                }
                if (screen == GameState::Game && isFinishShortcut(*keyEv)) {
                    triggerFinalSettlementNow();
                    continue;
                }
            }
            if (screen == GameState::Game) {
                if (const auto* keyEv = event.getIf<sf::Event::KeyPressed>()) {
            ...
            if (screen == GameState::Difficulty) {
                if (const auto* keyEv = event.getIf<sf::Event::KeyPressed>()) {
                    if (keyEv->code == sf::Keyboard::Key::Escape) {
                        difficultyPanel.setVisible(false);
                        screen = GameState::Title;
            ...
            if (screen == GameState::Game && !debugSandbox.isExpanded()
        ...
        // ── 持续性输入(移动) ─────────────────────────────────
        if (screen == GameState::Title) {
            titleMenuScreen.show();
            titleMenuScreen.update(dt);
            titleMenuScreen.render(window, tguiCtx);
            continue;
        }

        if (screen == GameState::Settings) {
            settingsMenuScreen.show();
            settingsMenuScreen.update(dt);
            settingsMenuScreen.render(window, tguiCtx);
            continue;
        }

        if (screen == GameState::Help) {
            helpMenuScreen.show();
            helpMenuScreen.update(dt);
            helpMenuScreen.render(window, tguiCtx);
            continue;
        }

        if (screen == GameState::Difficulty) {
            difficultyMenuScreen.show();
            difficultyMenuScreen.update(dt);
            difficultyMenuScreen.render(window, tguiCtx);
            continue;
        }

        if (sceneTransition.active) {
            sceneTransitionScreen.show();
            sceneTransitionScreen.render(window, tguiCtx);
            continue;
        }
        ...
        if (!debugSandbox.isExpanded() && !mealChoicePrompt.active && !activityNotice.active) {
            sf::Vector2f movementAxis = InputHandler::readMovementAxis();
            float dx = movementAxis.x;
            float dy = movementAxis.y;
        ...
        gamePlayScreen.render(window, tguiCtx);
    }
